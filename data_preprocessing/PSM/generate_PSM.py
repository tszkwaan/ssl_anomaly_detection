...
def main():
    ...
    # 1. Read CSV files (skip first column timestamp)
    # === Key fix: .fillna(0) to fill missing values ===
    ...
    # Read Labels
    # Note: If label file doesn't exist, generate dummy labels with all zeros
    ...
    # 2. Convert to Numpy and clean data (handle Infinity)
    # === Key fix: np.nan_to_num to handle infinity ===
    ...
    print(f"Train NaN check: {np.isnan(train_vals).any()}") # Should display False
